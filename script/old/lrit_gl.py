...
class LRIT:
  ...
  def next( self ):
    ...
    while True:
      ...
      if i != -1:
        while i < 1020:
          i += self.buffer.find( LRIT.frame_sync, i+1 )+1
          print "buffer position"+str(i)
        if i >= 1020:
          frame = LRIT.frame( LRIT.frame_sync + self.buffer[i-1020:i] )
          self.buffer = self.buffer[i+4:]
          return frame
      if self.mode == 'file':
        chunk = bytearray( self.fd.read( 2048 ) )
      else:
        chunk = bytearray( self.sock.recv( 2048 ) )
      self.buffer += chunk
